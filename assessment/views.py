    ...
    if 'userid' in request.session:
        ...
        if request.method=='POST':
            ...
            if form.is_valid():
                ...
                return render_to_response('assessment/notes.html',{'sett':sett,'varuser':varuser ,'varerr':varerr, 'session':session,'term':term,'klass':klass,'subject':subject})
        else:
            form= myform()
    ...
    if  "userid" in request.session:
        if request.is_ajax():
            if request.method == 'POST':
                ...
                post = request.POST.copy()
                acccode = post['userid']
                getdetails = tbltopic.objects.get(id = acccode)
                down = getdetails.lessonnote
                return render_to_response('assessment/academicrecord.html',{'getdetails':getdetails,'academic':academic})
